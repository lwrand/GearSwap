...
register_unhandled_command(function(...)
    ...
    for _,v in ipairs(cmds) do
        if S{'organizer','organize','org','o'}:contains(v:lower()) then
            org.export_set()
            return true
		elseif S{'org_ehn'}:contains(v:lower()) then
			org.export_set_ehn()
            return true
        end
    end
...
function org.export_set_ehn()
    ...
    for i,v in ipairs(flattab) do
		ward[ward_ids[ward_index]]:append(v)
		item_count = item_count + 1
		if (item_count > 79) then
			ward_index = ward_index + 1
			if (ward_index == 5) then
				windower.add_to_chat(123,'Organizer Library mod: max 320 items along 4 wardrobes!!!')
				return
			end
			item_count = 0
		end 
